...
NSString *httpUrl1 = @"http://apis.baidu.com/tngou/drug/search";
#define RADIUS 100.0
#define PHOTONUM 5
...
- (void)configViews {
    ...
    NSArray *dataArray = @[@"xinzang", @"xinzang", @"xinzang", @"xinzang", @"xinzang"];
    
    CGFloat centery = self.view.center.y  + 150;
...
- (void)didTapped:(NSInteger)index {
    ...
    if (self.currentTag  == index) {
        ...
        if (index == 1001) {
            ...
            NSString *typeString = (__bridge NSString *)CFURLCreateStringByAddingPercentEscapes(NULL,(__bridge CFStringRef)@"头痛",NULL,(CFStringRef)@"!*'();:@&=+$,/?%#[]",kCFStringEncodingUTF8);
            
            NSString *urlString = [NSString stringWithFormat:@"name=drug&keyword=%@&page=1&rows=30&type=name",typeString];
            [[MedicineHelper sharedManager] request:httpUrl1 withHttpArg:urlString];
        ...
        if (index == 1000) {
            ...
            NSString *typeString = (__bridge NSString *)CFURLCreateStringByAddingPercentEscapes(NULL,(__bridge CFStringRef)@"心",NULL,(CFStringRef)@"!*'();:@&=+$,/?%#[]",kCFStringEncodingUTF8);
